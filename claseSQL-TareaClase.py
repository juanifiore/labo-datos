...
def main():
    ...
    vuelo      = pd.read_csv("~/Documentos/labo/vuelo.csv")    
    aeropuerto = pd.read_csv("~/Documentos/labo/aeropuerto.csv")    
    pasajero   = pd.read_csv("~/Documentos/labo/pasajero.csv")    
    reserva    = pd.read_csv("~/Documentos/labo/reserva.csv")    
    # Ejercicio JOIN tuplas espúreas
    empleadoRol= pd.read_csv("~/Documentos/labo/empleadoRol.csv")    
    rolProyecto= pd.read_csv("~/Documentos/labo/rolProyecto.csv")    
    # Ejercicios funciones de agregación, LIKE, Elección, Subqueries y variables de Python
    examen     = pd.read_csv("~/Documentos/labo/examen.csv")
    # Ejercicios de manejo de valores NULL
    examen03 = pd.read_csv("~/Documentos/labo/examen03.csv")
    
    # EJERCICIOS PARTICULARES
    
    print("# =============================================================================")
    print("# Ejercicios alumnos")
    print("# =============================================================================")
    
    consigna    = " contar examenes"
    consultaSQL = """
                   SELECT count(*) AS cantidadExam
                   FROM examen
                  """
    imprimirEjercicio(consigna, [examen,examen03], consultaSQL)    
    
    
    
        
    consigna    = " separa en grupos"
    consultaSQL = """
                   SELECT instancia,count(*) AS asistieron
                   FROM examen
                   GROUP BY instancia
                   ORDER BY asistieron ASC;
                  """
    imprimirEjercicio(consigna, [examen,examen03], consultaSQL)    
    
    
    
        consigna    = " asistieron menos de 4"
    consultaSQL = """
                   SELECT instancia,count(*) AS asistieron
                   FROM examen
                   GROUP BY instancia
    ...
    imprimirEjercicio(consigna, [examen,examen03], consultaSQL)    
           ...
           consigna    = " PROMEDIO EDAD"
    consultaSQL = """
                   SELECT instancia,AVG(edad) AS edadProm
                   FROM examen
                   GROUP BY instancia
                   ORDER BY edadprom
    ...
    imprimirEjercicio(consigna, [examen,examen03], consultaSQL)    
           ...
           consigna    = " PROMEDIO notas parcial"
    consultaSQL = """
                   SELECT instancia,AVG(nota) AS notaProm
                   FROM examen
                   GROUP BY instancia
    ...
    imprimirEjercicio(consigna, [examen,examen03], consultaSQL)    
     ...
     print()
    print("# =============================================================================")
    print("# Ejercicios AEROPUERTO")
    print("# =============================================================================")
    
    consigna    = "a.- Listar CODIGO Y NOMBRE de los aeropuertos de londres "
    consultaSQL = """
                   SELECT DISTINCT DNI, Salario
                   FROM empleado;
                  """
    imprimirEjercicio(consigna, [vuelo,aeropuerto,pasajero,reserva], consultaSQL)    
    ...
    consultaSQL = """
                   ...
                   FROM empleado;
                  """
    imprimirEjercicio(consigna, [empleado,alumnosBD,nacionalidades,materia], consultaSQL)    
    ...
    consigna    = "que retorna CODIGOS DE VUELO CON RESERVAS GENERADAS"
    consultaSQL = """
                   SELECT numero
                   FROM vuelo
                   INTERSECT
                   ...
                   FROM reserva;
                  """
    imprimirEjercicio(consigna, [vuelo,reserva,pasajero,nacionalidades], consultaSQL)    
    ...
    consigna    = "codigos de los aeropuertos de los que parten o arriban vuelos"
    consultaSQL = """
                   SELECT origen
                   FROM vuelo
                   UNION
    ...
    consigna    = "nombre de la ciudad de partidad el vuelo 165"
    consultaSQL = """
                   SELECT ciudad
                   FROM aeropuerto
                   INNER JOIN vuelo
    ...
    consigna    = "nombre de las personas con reservas de valor menor a 200"
    consultaSQL = """
                   SELECT nombre
                   FROM pasajero
                   INNER JOIN reserva
    ...
    consigna    = "NOMBRE FECHA Y DESTINO DE LOS PASAJEROS DESDE MADRID"
    consultaSQL = """
                   SELECT pasajero.nombre,fecha,ciudad,
                   FROM pasajero,reserva,vuelo,aeropuerto
                   WHERE pasajero.dni = reserva.dni AND nrovuelo = numero AND origen = codigo AND origen = 'MAD';
                  """
    imprimirEjercicio(consigna, [vuelo,reserva,pasajero,nacionalidades], consultaSQL)    
    ...
    print()
    print("# =============================================================================")
    print("# Ejercicios AR-UNION, INTERSECTION, MINUS <-> UNION, INTERSECT, EXCEPT")
    print("# =============================================================================")
